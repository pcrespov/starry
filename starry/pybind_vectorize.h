...
namespace pybind_vectorize {
    ...
    inline Vector<double> vectorize_arg(const py::array_t<double>& arg,
        ...
        if (arg.size() == 1) {
            ...
            if ((size == 0) || (size_t(res.size()) == size)) {
                size = res.size();
                return res;
            } else {
    ...
    //! Vectorized `fluence` method: single-wavelength starry
    template <typename T>
    typename std::enable_if<!std::is_base_of<Eigen::EigenBase<Row<T>>,
                                             Row<T>>::value, py::object>::type
    fluence(maps::Map<T> &map, py::array_t<double>& theta, double& exposure,
        ...
        if (gradient) {
            ...
            // 761#issuecomment-288818460
            int t = 0;
            auto F = py::vectorize([&map, &grad, &t, &dF_names, &numerical, &exposure]
                        (double theta) {
                // Evaluate the function
                double res = static_cast<double>(map.fluence(theta, exposure, true,
                                                             numerical));
                // Gather the derivatives
                auto dF = map.getGradient();
                int n_ylm = 0, n_ul = 0;
            ...
            // Easy! We'll just return F
            return py::vectorize([&map, &numerical, &exposure](double theta) {
                return static_cast<double>(map.fluence(theta, exposure, false,
                                           numerical));
            })(theta);
    ...
    //! Vectorized `fluence` method: spectral starry
    template <typename T>
    typename std::enable_if<std::is_base_of<Eigen::EigenBase<Row<T>>,
                                            Row<T>>::value, py::object>::type
    fluence(maps::Map<T> &map, py::array_t<double>& theta, double& exposure, bool gradient,
            bool numerical){

        // Vectorize the arguments manually
        size_t sz = theta.size();
        Vector<double> theta_v = vectorize_arg(theta, sz);
        ...
        if (gradient) {
            ...
            for (size_t i = 0; i < sz; ++i) {
                ...
                // Function value
                F.row(i) = map.fluence(theta_v(i), exposure, true,
                           numerical).template cast<double>();
            ...
            Matrix<double> F(sz, map.nwav);
            for (size_t i = 0; i < sz; ++i) {
                F.row(i) = map.fluence(theta_v(i), exposure, false,
                           numerical).template cast<double>();
            }
